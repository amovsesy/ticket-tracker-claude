...
export async function searchUsers(db: ReturnType<typeof drizzle>, searchTerm: string) {
	return await db
		.select()
		.from(users)
		.where(or(like(users.email, `%${searchTerm}%`), like(users.name, `%${searchTerm}%`)))
		.all();
}
...
export async function getPublishedPostsAfterDate(db: ReturnType<typeof drizzle>, date: Date) {
	return await db
		.select()
		.from(posts)
