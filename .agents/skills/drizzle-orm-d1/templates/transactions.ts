...
export async function DONT_useTraditionalTransaction(db: ReturnType<typeof drizzle>) {
	try {
		await db.transaction(async (tx) => {
			...
			await tx
				.insert(posts)
				.values({ title: 'Post', slug: 'post', content: 'Content', authorId: 1 });
		});
	} catch (error) {
...
export async function batchInsertUsers(
	...
	const statements = usersData.map((user) => db.insert(users).values(user).returning());
...
export async function batchReadOperations(db: ReturnType<typeof drizzle>, userIds: number[]) {
	const queries = userIds.map((id) => db.select().from(users).where(eq(users.id, id)));
...
export async function bulkInsertUsers(
	...
	for (let i = 0; i < usersData.length; i += BATCH_SIZE) {
		...
		const statements = chunk.map((user) => db.insert(users).values(user).returning());
